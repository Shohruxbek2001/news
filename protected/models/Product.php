...
//         	'filesBehavior'=>'\file\behaviors\FilesBehavior',
            ...
            if (count($attr_filter) > 0) {
                ...
                if (!empty($values)) {
                    ...
                    if (!empty($eavValues)) {
                        ...
                        if (empty($ids)) $criteria->addCondition('1<>1');
                        else $criteria->addInCondition('`t`.`id`', $ids);
                    }
                }
            }
        }
    ...
    /**
     * Фильтрация по атрибутам через их пересечение
     * @return array|CDbCommand|mixed
     */
    public static function eavCriteria()
    {
        $ids = [];
        $attributes = Yii::app()->request->getQuery('data', []);
        $attributes = json_decode($attributes, true);
        if ($attributes) {
            $queryEav = 'SELECT `id_product` FROM `eav_value` as a WHERE ';
            $conditions = [];
            $conditions2 = [];
            foreach ($attributes as $key => $attributeValues) {
                if (!isset($conditions[$attributeValues['name']])) {
                    $conditions[$attributeValues['name']] = [];
                }
                $conditions[$attributeValues['name']] [] = '(`a`.`value` = "' . $attributeValues['value'] . '" AND `a`.`id_attrs` = "' . $attributeValues['name'] . '")';
            }
            if (!empty($conditions)) {
                foreach ($conditions as $key => $condition) {
                    $conditions2[] = implode(' OR ', $condition);
                }
            }
            $idx = 0;
            foreach ($conditions2 as $k => $cond) {
                $idx++;
                $query1 = $queryEav . $cond . ' GROUP BY id_product';
                $ids[$idx] = HDb::queryColumn($query1);
            }
            $ids = count($ids) === 1
                ? $ids[1]
                : array_intersect(...$ids);
    ...
    public function search($pageSize = false, $returnCriteria = false)
    {
        ...
        if ($price_from && $price_to) {
            ...
            $criteria->addCondition('price >= ' . $price_from, 'AND');
        } elseif ($price_to) {
        ...
        if (D::cms('shop_enable_filter')) {
            $attributesCriteriaIDs = $this->eavCriteria();

            if (count($attributesCriteriaIDs)) {
                $criteria->addInCondition('t.id', $attributesCriteriaIDs);
            }
        }

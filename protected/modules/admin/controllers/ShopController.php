...
class ShopController extends AdminController
{
    ...
    public function actionProductCreate($category_id = null)
    {
        ...
        [$fixAttributes, $eavAttributes, $productAttrs] = $this->getAttributes($model);

        $this->render('productcreate', compact('model', 'fixAttributes', 'eavAttributes', 'productAttrs'));
    }
    ...
    public function actionProductUpdate($id, $price = null, $save = false)
    {
        ...
        if (isset($_POST['Product'])) {
            $model->attributes = $_POST['Product'];
            if ($model->save()) {
                if (!empty($_POST['EavValue'])) {
                    foreach ($_POST['EavValue'] as $key => $value) {
                        if ($eavAttributes[$key]->type == EavAttribute::SINGLE) {
                            if (!isset($productAttrs[$key])) {
                                $attributesProduct = new EavValue;
                                $attributesProduct->id_attrs = $key;
                                $attributesProduct->id_product = $model->id;
                                $attributesProduct->value = $value;
                                $attributesProduct->save();
                            } elseif ($productAttrs[$key] != $value) {
                                $eav = EavValue::model()->find('id_attrs=:id_attrs and id_product=:id_product', [':id_attrs' => $key, ':id_product' => $model->id]);
                                $eav->value = $value;
//                                print_r($eav);
//                                die('aaa');
                                $eav->save();
                            ...
                            $eavs = EavValue::model()->findAll('id_attrs=:id_attrs and id_product=:id_product', [':id_attrs' => $key, ':id_product' => $model->id]);
                            $exist = [];
                            foreach ($eavs as $eav) {
                                if (!in_array($eav->value, $value)) {
                                    $eav->delete();
                                } else {
                                    $exist []= $eav->value;
                                }
                ...
                $url = $this->createUrl('shop/productupdate', array('id' => $model->id));
                $this->redirect($url);
            }
        }
        ...
        $this->render('productupdate', compact('model', 'fixAttributes', 'categoryList', 'relatedCategories', 'eavAttributes', 'productAttrs'));
    }
    ...
    protected function getAttributes($model)
    {
        ...
        if (Yii::app()->params['attributes']) {
            ...
            $fixAttributes = EavAttribute::model()->with('dictionary')->findAll($criteria);
            $allAttributes = EavAttribute::model()->with('dictionary')->findAll();
            foreach ($allAttributes as $eavAttr) {
                $eavAttributes[$eavAttr->id] = $eavAttr;
            }
            if ($model->isNewRecord) {
                foreach ($fixAttributes as $fx) {
                    $productAttrs[$fx->id] = null;
                }
            } else {
                foreach ($model->productAttributes as $pAttr) {
                    $isDictionary = count($eavAttributes[$pAttr->id_attrs]) ? true : false;
                    if (!$isDictionary) {
                        $productAttrs[$pAttr->id_attrs] = $pAttr->value;
                    } else if (!isset($productAttrs[$pAttr->id_attrs])) {
                        $productAttrs[$pAttr->id_attrs] = [];
                        $productAttrs[$pAttr->id_attrs][] = $pAttr->value;
                    } else {
                        $productAttrs[$pAttr->id_attrs][] = $pAttr->value;
                    }
        ...
        return [$fixAttributes, $eavAttributes, $productAttrs];
    }

    public function actionRemoveProductAttribute()
    {
        $id_attrs = Yii::app()->getRequest()->getParam('id_attrs');
        $id_product = Yii::app()->getRequest()->getParam('id_product');
        if(Yii::app()->request->isAjaxRequest && $id_product && $id_attrs) {
            EavValue::model()->deleteAll('id_attrs=:id_attrs and id_product=:id_product', [':id_attrs' => $id_attrs, ':id_product' => $id_product]);
            $result = ['msg' => 'success'];

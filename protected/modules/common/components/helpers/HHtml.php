...
class HHtml
{
	 ...
	 * @param string $point символ разделения рублей и копеек. 
	 * По умолчанию "," (запятая).
	 * @param string $thousands символ разделения тысячных.
	 * По умолчанию пробел.
	 * @return string 
	 */
	public static function price($price, $point=',', $thousands=' ')
	{
		...
		return number_format($price, $decimal, $point, $thousands);
	}
	 ...
	 * Может быть передан атрибут "label" для замены текста номера телефона.
	...
	{
	    if($phone) {
	    	if(A::existsKey($htmlOptions, 'label')) {
	        ...
	        return \CHtml::link($label, 'tel:'.preg_replace(['/[^+0-9]+/', '/^8/', '/^([^+])/'], ['', '+7', '+7$1'], $phone), $htmlOptions);
	    }
	    return $default;
	}
	...
	/**
	 * Отображение HTML блока после полной загрузки страницы
	 * @param string $jsCssClass имя CSS класса на который навешивается обработчик
	 * @param string $htmlOptions дополнительные HTML атрибуты для тэга блока ожидания загрузки.
	 * Доступны дополнительные параметры:
	 * "tag" - тэг блока ожидания загрузки. По умолчанию DIV.
	 * "message" - сообщение ожидания загрузки. По умолчанию "Подождите, идет загрузка..."
	 * @return string тэг блока ожидания загрузки
	 */
	public static function onReady($jsCssClass, $htmlOptions=['class'=>'alert alert-default'])
	{
		$id=HHash::u('js-loader');
		
		Y::js($id, ';$(".'.$id.'").hide();$(".'.$jsCssClass.'").show();', \CClientScript::POS_READY);

		if($tag=A::get($htmlOptions, 'tag')) {
			unset($htmlOptions['tag']);
		}
		if($message=A::get($htmlOptions, 'message')) {
			unset($htmlOptions['message']);
		}

		$htmlOptions['class']=trim(A::get($htmlOptions, 'class', '') . ' ' . $id);

		return \CHtml::tag($tag?:'div', $htmlOptions, $message?:'Подождите, идет загрузка...');
	}

	/**
	 * Получить ссылку на электронную почту
	 * @param []|string массив или строка списка адресов электронной почты, разделенных пробелом 
	 * или символами переданными в параметре $explodeChars.
	 * @param string $separator разделитель адресов электронной почты. По умолчанию ",(пробел)".
	 * @param [] $linkOptions дополнительные HTML атрибуты для формирования ссылки.
	 * @param string $explodeChars символы разделения входящих адресов электронной почты, если
	 * они переданы строкой.
	 * @return string
	 */
	public static function emailLink($emails, $separator=', ', $linkOptions=[], $explodeChars=',')
	{
		if(is_string($emails)) {
			$emails=explode(' ', preg_replace('/[\s'.addcslashes($explodeChars, '[]-\\/').']+/', ' ', trim($emails)));
		}
		return implode($separator, array_map(function($email) use ($linkOptions) { 
			return \CHtml::link($email, 'mailto:' . $email, $linkOptions); 
		}, $emails));
	}
}

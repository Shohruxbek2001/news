...
class HFile
{
	 ...
	 * @param string|null $downloadFilename имя файла для загрузки (локальное)
	 */
	public static function download($filename, $content=null, $allowEmpty=false, $deniedFile=false, $callback=null, $downloadFilename=null)
	{
		$filename=preg_replace('#/+#', Y::DS, urldecode($filename));
		
		$contentDispositionFilename=null;
		if($content !== null) {
		    $contentDispositionFilename=$downloadFilename ?: pathinfo($filename, PATHINFO_BASENAME);
     ...
     * @param array $data
     */
    public static function csv($file, $data, $delimiter=';', $enclosure='"', $escape_char='\\', $withBOM=false, $mode='w+')
    {
	 ...
	 *
	 * @param string $filename путь к файлу CSV.
	 * @param integer $limit максимальное количество строк. 
	 * По умолчанию 0 (нуль) - без ограничений.
	 * @param integer $offset сдвиг. По умолчанию 0 (нуль).
	 * @param string $delimiter символ разделения. По умолчанию ";".
	 * @param string $enclosure символ обрамления. По умолчанию '"'.
	 * @param string $escape_char символ отмены. По умолчанию "\\".
	 * @return []|null
	 */
	public static function readCsv($filename, $limit=0, $offset=0, $delimiter=';', $enclosure='"', $escape_char='\\')
	{
		$data=null;

		if (($handle = fopen($filename, "r")) !== false) {
			$data = [];
			$max=$limit ? ($limit + $offset - 1) : 0;
			$n=-1;
    		while (($row = fgetcsv($handle, 0, $delimiter, $enclosure, $escape_char)) !== false) {
				$n++;
				if($max && $offset && ($n < $offset)) continue;
				$data[]=$row;
				if($max && ($n == $max)) break;
			}
			fclose($handle);
	...
	/**
	 * Получить количество строк CSV файла
	 *
	 * @param string $filename путь к файлу CSV.
	 * @param string $delimiter символ разделения. По умолчанию ";".
	 * @param string $enclosure символ обрамления. По умолчанию '"'.
	 ...
	 * @return int
	 */
	public static function getCsvCountLines($filename, $delimiter=';', $enclosure='"', $escape_char='\\')
	{
		$count=0;
		foreach(static::readCsvIterator($filename, $delimiter, $enclosure, $escape_char) as $row) {
			$count++;
		}
		return $count;
    }
	...
	/**
	 * Получить итератор чтения данных из CSV файла
	 *
	 * @param string $file путь к файлу CSV.
	 * @param string $delimiter символ разделения. По умолчанию ";".
	 * @param string $enclosure символ обрамления. По умолчанию '"'.
	 * @param string $escape_char символ отмены. По умолчанию "\\".
	 * @return [] 
	 */
	public static function readCsvIterator($filename, $delimiter=';', $enclosure='"', $escape_char='\\')
	{
		if (($handle = fopen($filename, "r")) !== false) {
			while(!feof($handle)) {
				yield fgetcsv($handle, 0, $delimiter, $enclosure, $escape_char);
			}
			fclose($handle);
		}
    }

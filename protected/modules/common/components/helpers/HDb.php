...
class HDb
{
	...
	public static function insert($tableName, $data, $duplicate=null, $default=[], $db=null)
	{
	    ...
	    foreach($data as $record) {
	        ...
	        foreach($fields as $attribute=>$idx) {
	            ...
	            $_values[]=(is_numeric($value) && (strpos($value, '0') !== 0)) ? $value : (is_null($value) ? 'NULL' : static::qv($value));
	        }
	 ...
	 * @param string $tableName имя таблицы
	 * @param string $attributeValue имя поля значения. По умолчанию "title".
	 * @param string $attributeKey имя поля ключа. По умолчанию "id".
	 * @param [] $options дополнительные параметры для запроса. 
	 * Доступны следующие параметры:
	 * "where" - дополнительное условие в блок WHERE
	 * "params" - параметры для запроса
	 * @return [] массив вида array(key=>value)
	 */
	public static function listData($tableName, $attributeValue='title', $attributeKey='id', $options=[])
	{
		$data=[];

		$query='SELECT ' . static::qc($attributeKey) . ', ' . static::qc($attributeValue) . ' FROM '
			. static::qt($tableName);
		...
		$items=static::queryAll($query, A::get($options, 'params', []));
		foreach($items as $item) {
			$data[$item[$attributeKey]]=$item[$attributeValue];
		}

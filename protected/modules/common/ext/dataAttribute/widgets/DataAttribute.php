...
class DataAttribute extends \common\components\base\Widget
{
	...
	public function generateRow($index, $data=array()) 
	{
		...
		foreach($this->header as $key=>$title) {
			$type=A::get($this->types, $key) ?: 'string';
			$rowHtmlOptions=A::rget(A::toa($type), 'params.cellOptions', []);
			$htmlRow='';
			...
			if(in_array($key, $this->readOnly)) {
				if(is_string($value)) {
					...
					$htmlRow .= $value . \CHtml::tag('div', ['style'=>'font-size:0.8em;opacity:0.7;font-style:italic'], A::get(A::get($this->notes, $index, []), $key, ''));
				}
			}
			else {
				$typeData=null;
				...
				if($type=A::get($this->types, $key)) {
					if(is_array($type)) {
						...
						$types=A::get($type, 'types');
						$type=A::get($type, 'type');

						if(!$type && !empty($types)) {
							$defaultType=A::get($types, 'default', 'string');
							$type=$isTemplate ? $defaultType : A::get($types, $index, $defaultType);
							if(is_array($type)) {
								$typeData=A::get($type, 'data');
								$typeDefault=A::get($type, 'default'); 
								$typeView=A::get($type, 'view');

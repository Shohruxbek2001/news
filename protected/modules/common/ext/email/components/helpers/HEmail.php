...
class HEmail
{
     ...
     * версии тела письма
     * @param [] $attachFiles прикрепляемые файлы
     * 
     * @return bool
     */
    public static function send($config, $data, $bodyView=false, $altBodyView=false, $attachFiles=[])
    { 
    ...
    public static function cmsSend($to, $subject, $data, $bodyView=false, $altBodyView=false, $attachFiles=[])
    {
        ...
        return static::send($config, $data, $bodyView, $altBodyView, $attachFiles);
    }
    
    public static function cmsAdminSend($subject, $data, $bodyView=false, $altBodyView=false, $customEmail=null, $attachFiles=[])
    {
        if($subject === true) {
            $subject='Новое сообщение с сайта ' . \D::cms('sitename', $_SERVER['SERVER_NAME']);
        }
        $config=static::config();
        $config['address']['to']=[static::normalizeAddress(A::toa($customEmail ?: \D::cms('email')))];
        $config['ishtml']=true;
        $config['subject']=$subject;

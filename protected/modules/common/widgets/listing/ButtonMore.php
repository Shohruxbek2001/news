...
class ButtonMore extends \CWidget
{
    ...
    /**
     * Дополнительный js скрипт перед получением данных
     * @var string|null
     */
    public $jsBeforeLoading=null;

    /**
     * Дополнительный js скрипт выполняемый сразу после 
     * получения данных.
     * @var string|null
     */
    public $jsAfterLoading=null;
    ...
    public function run()
    {
        ...
        if(($this->url || $this->jsGetUrl) && $this->container) {
            if($this->pageCount > 1) {
                ...
                $this->htmlOptions['data-page-count']=$this->pageCount;
    ...
    public function registerScript($jsId)
    {
        ...
        \$(document).on("click", "[data-js='{$jsId}']", function(e){ 
            e.preventDefault();
            function btn() { return \$("[data-js={$jsId}]"); }            
EOJS;

		if($this->jsBeforeLoading) $script.='(function(){'.$this->jsBeforeLoading.'})();';
        else $script.='btn().addClass("loading");';
...
EOJS;
		...
		if($this->jsAfterLoading) $script.='(function(){'.$this->jsAfterLoading.'})();';
        else $script.='btn().removeClass("loading");';
...
EOJS;
        ...
        else $isLastPageExpression="((+btn().attr('data-page') + 1) > +btn().attr('data-page-count'))";
        
        $script.=<<<EOJS
                if($isLastPageExpression){ btn().hide(); btn().parent().hide(); }

...
class DefaultController extends BaseController
{
	...
	public function filters()
	{
        ...
        if($cid=A::get($_REQUEST, 'cid')) {
            $filters=HCrud::param($cid, 'controllerFilters', []);
        }
	...
	{
	    if($onBeforeLoad=HCrud::param($cid, 'crud.onBeforeLoad')) call_user_func($onBeforeLoad, $cid);
	    if($onBeforeLoad=HCrud::param($cid, 'crud.delete.onBeforeLoad')) {
	        call_user_func($onBeforeLoad, $cid);
        }
		
		$model=HCrud::getById($cid, $id, null, true, HCrud::param($cid, 'crud.index.scenario', 'delete'));
        $slider_id = get_class($model) === 'extend\modules\slider\models\Slide'
            ? $model->slider_id
            : false;
        ...
        $slider_id
            ? $this->redirect(HCrud::getConfigUrl($cid, 'crud.index.url', '/crud/admin/default/index', ['cid'=>$cid, 'slider'=>$slider_id], 'c'))

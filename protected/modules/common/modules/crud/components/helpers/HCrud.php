...
class HCrud
{
	 ...
	 */
	public static function getMenuItems($controller, $id=null, $baseUrl='/crud/admin/default/index', $returnItem=false, $visible=true)
	{
		$items=[];
		...
		if($config=static::config()) {
			...
			$fAddItem=function($id, $params) use (&$items, $controller, $baseUrl, $config, $visible) {
				if(!A::rget($params, 'menu.backend.disabled', false)) {
					if($label=A::rget($params, 'menu.backend.label', A::rget($params, 'crud.index.title'))) {
						$items[]=[
						    'visible'=>is_callable($visible) ? call_user_func($visible) : $visible,
							'label'=>$label,
							'url'=>[$baseUrl, 'cid'=>$id],

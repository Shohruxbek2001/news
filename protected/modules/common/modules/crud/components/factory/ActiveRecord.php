...
class ActiveRecord
{
                ...
                if($classConfig = A::get($config, 'config')) {
                    if ($class === trim(A::get($config, 'class', ''), '\\')) {
                        ...
                        if(!$tableSchema) {
                            ...
                            static::createIndexes($tableName, A::get($classConfig, 'indexes', []));
    ...
    protected static function createIndexes($tableName, $indexes=[])
    {
        if(!empty($indexes)) {
            $default=[
                'name'=>null,
            ...
            foreach($indexes as $name=>$params) {
                ...
                if(is_numeric($name)) {
                    ...
                    $index=A::m(['name'=>$name], $params);
                }

                $index=A::m($default, $index);
                if($index['name']) {
                    $index['name']=mb_strtolower($index['name']);
                    if(empty($index['columns'])) {
                        $index['columns']=[$index['name']];
                    }
                    if($index['references_table'] && !empty($index['references_columns'])) {
                        $foreigns[$index['name']]=$index;
                    }
                    elseif(!$index['references_table']) {
            ...
            $tableIndexes=HDb::queryAll("SHOW INDEX FROM `{$tableName}`");
            foreach($tableIndexes as $indexColumns) {
                foreach($indexColumns as $name=>$value) {
                    if(mb_strtolower($name) === 'column_name') {
                        $indexName=mb_strtolower($value);
            ...
            if(!empty($foreigns)) {
                ...
                foreach($tableForeigns as $foreignName) {
                    if(isset($foreigns[mb_strtolower($foreignName)])) {
                        unset($foreigns[$foreignName]);
                    }
                }
            }
            ...
            foreach($indexes as $index) {
                $queries[]="ALTER TABLE `{$tableName}` ADD " 
                    . ($index['unique'] ? 'UNIQUE ' : '')
                    . "INDEX `{$index['name']}` (`" . implode('`, `', $index['columns']) . '`)';
            }
            foreach($foreigns as $foreign) {
                $queries[]="ALTER TABLE `{$tableName}` ADD FOREIGN KEY" 
                    . " `{$foreign['name']}` (`" . implode('`, `', $foreign['columns']) . '`)'
                    . " REFERENCES `{$foreign['references_table']}` (`" . implode('`, `', $foreign['references_columns']) . '`)'
                    . ($index['on_update']==='' ? '' : (' ON UPDATE ' . ($index['on_update']?:'CASCADE')))
                    . ($index['on_delete']==='' ? '' : (' ON DELETE ' . ($index['on_delete']?:'CASCADE')));
            }

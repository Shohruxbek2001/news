...
class DApi extends \CComponent
{
	...
	public function init()
	{
        if(!static::$_initialized) {
			...
			// Fix: ошибка режима сombine Yii::CClientScript в том, что он регулярно создает 
			// новые файлы объединения, не удаляя старые, тем самым заполняя пространство хостинга.
			if((int)date('w') === 6) {
				$checkFile=\Yii::getPathOfAlias('application.runtime') . DS . '.fix-clientscript';
				if(!is_file($checkFile) || (date('Y-m-d', filemtime($checkFile)) != date('Y-m-d'))) {
					HFile::readDir(\Yii::getPathOfAlias('webroot.assets'), false, function($dirname, $entry, $params) {
						if(preg_match('/^(script|style)-.+\.(css|js)$/i', $entry)) {
							unlink(HFile::path([$dirname, $entry]));
						}
					});
    ...
    /**
     * Регистрация модуля "Статьи"
     */
    private function _registerArticles()
    {
        // добавление пункта меню
        $this->_addMenuItem(array(
            'title'=>'Статьи',
            'options'=>'{"model":"articles"}'
        ));
    }

    /**
     * Регистрация модуля "Услуги"
     */

...
class DShopRule extends CBaseUrlRule
{
	...
	protected function getByRoute($className, $route, $criteria=null)
	{
		...
		if(is_numeric($route)) {
			if($model=$this->findByAlias($className, $route, $criteria)) {
				return $model;
			}
			else {
				return $className::model()->findByPk($route, ['select'=>$criteria->select]);
			}
		}

		return $this->findByAlias($className, $route, $criteria);
	}

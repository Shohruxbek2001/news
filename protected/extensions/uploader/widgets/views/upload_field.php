        ...
        formData: {hash: "<?=$this->hash?>"},
            ...
            if(data.result.error==1){
                ...
                if("jpg,jpeg,gif,png".indexOf(data.result.ext) > -1) {
                    $('.form-upload_<?=$this->hash?> .image_place').append('<a href="javascript:;" data-src="'+data.result.img+'" data-fancybox="1" rel="form-<?= $this->id; ?>"><i data-src="'+data.result.img+'" class="js-file-delete fas fa-times-circle" title="Удалить">X</i><img src="'+data.result.img+'"></a>');
                }
                else {
